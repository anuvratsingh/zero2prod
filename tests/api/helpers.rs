...
pub async fn spawn_app() -> TestApp {
    ...
    let configuration = {
        ...
        c.email_client.base_url = email_server.uri(); // Use mock serve as email api
        c
    };
    configure_database(&configuration.database).await;

    let applicaiton = Application::build(configuration.clone())
        .await
        .expect("Failed to build application");

    let application_port = applicaiton.port();

    let _ = tokio::spawn(applicaiton.run_until_stopped());
